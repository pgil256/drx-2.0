...
class ResetWorker(QRunnable):
    ...
    def _wait_for_done(self, timeout=10.0, operation_name="operation"):
        ...
        # The flag (I2Cstatus) must be reset to 0 in the 'run' method *before* calling this

        while self.main_window.I2Cstatus == 0 and time.time() - start_wait < timeout:
            # Polling the flag via the reference
            time.sleep(0.05) # Short sleep *in worker thread* is acceptable
        ...
        if self.main_window.I2Cstatus == 1:
            ...
            self.main_window.I2Cstatus = 0 # Reset flag via reference for next operation
            return True
        else: # Timeout occurred
            print(f"Worker timeout waiting for '{operation_name}' completion.")
            self.main_window.I2Cstatus = 0 # Ensure flag is reset on timeout too
            return False
