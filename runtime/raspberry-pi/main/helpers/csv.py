...
class CSVHelper:
    ...
    def _seed_users_file(self, users_file):
        ...
        try:
            ...
            with open(users_file, "x", encoding="utf-8", newline="") as stream:
                csv.writer(stream).writerow(["pin_hash", "username", "email", "status"])
            self.logger.info("Seeded empty users file at %s", users_file)
        except FileExistsError:
            pass
        except OSError as e:
            self.logger.error("Could not seed users file at %s: %s", users_file, e)
