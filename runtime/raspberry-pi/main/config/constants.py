...
from .paths import APP_BASE_DIR, DEVICE_STATE_DIR, PROJECT_DIR
...
DATA_PATHS = {
    ...
    "USER_PINS": os.environ.get(
        ...
        os.path.join(DEVICE_STATE_DIR, "data/user_pins.csv"),
    ),
    ...
    "AUTH_STATE": os.environ.get(
        ...
        os.path.join(DEVICE_STATE_DIR, "data/auth_state.json"),
    ),
    "PENDING_UPLOADS": os.environ.get(
        "KNEESPA_PENDING_UPLOADS_PATH",
        os.path.join(DEVICE_STATE_DIR, "data/pending_uploads.json"),
    ),
}
...
# Device state is outside runtime/, so replacing software cannot overwrite it.
DEFAULT_CONFIG_PATH = os.path.join(DEVICE_STATE_DIR, "config", "kneespa.cfg")
LEGACY_CONFIG_PATH = (
    "" if os.environ.get("KNEESPA_DEVICE_DIR")
    or os.path.exists(os.path.join(DEVICE_STATE_DIR, ".legacy-migrated"))
    else os.path.join(PROJECT_DIR, "main", "config", "kneespa.cfg")
)
...
# Paired with motor.ino's jerkInterval initializer (development/scripts/check_limits_sync.py).
