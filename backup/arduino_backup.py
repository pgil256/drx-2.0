    ...
    def reset_dtr(self):
        ...
        try:
            if not self.serial_com:
                print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] DTR Reset FAILED - No serial connection")
                return False

            print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Starting DTR Reset sequence...")
            start_time = time.time()

            # Save current DTR state
            original_dtr = self.serial_com.dtr
            print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Original DTR state: {original_dtr}")

            # Toggle DTR
            self.serial_com.dtr = False
            print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] DTR set to FALSE")
            time.sleep(0.1)  # Brief delay
            self.serial_com.dtr = True
            print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] DTR set to TRUE - Waiting 5s for Arduino init...")
            time.sleep(5)  # Allow Arduino to initialize

            # Restore original DTR state
            self.serial_com.dtr = original_dtr
            elapsed = time.time() - start_time
            print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] DTR Reset COMPLETE - Elapsed: {elapsed:.2f}s, DTR restored to: {original_dtr}")

            return True

        except Exception as e:
            print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] DTR Reset ERROR: {e}")
            return False
