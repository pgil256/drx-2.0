...
class TestLoadCsvHeadlessRobustness:
    ...
    def test_initialize_data_survives_missing_csv(
        ...
        empty users file and loads zero users. It
        ...
        assert helper.users == {}
        # First-run seeding creates an empty operator table.
        assert csv_module.os.path.exists(missing)

    def test_initialize_data_missing_csv_and_template(
        self, clean_auth_env, no_qapplication, monkeypatch, tmp_path
    ):
        """A runtime-only deployment can initialize without provisioning templates."""
        ...
        assert helper.users == {}
        assert real_exists(missing)
        with open(missing, encoding="utf-8") as stream:
            assert stream.read().splitlines() == ["pin_hash,username,email,status"]
