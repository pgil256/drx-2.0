...
def read_isolated_constants(overrides: Dict[str, str]) -> dict:
    ...
    result = subprocess.run(
        [sys.executable, "-c", "\n".join([
            "import json, sys",
            "sys.path.insert(0, 'runtime/raspberry-pi/main')",
...
class TestEnvironmentOverrides:
    ...
    def test_base_directory_controls_default_paths(self, tmp_path: Path) -> None:
        ...
        state = base.parent / "devices" / "local" / "raspberry-pi"
        assert Path(values["CONFIG_PATH"]) == state / "config" / "kneespa.cfg"
        assert Path(values["LOG_DIR"]) == state / "logs"
        assert {key: Path(value) for key, value in values["DATA_PATHS"].items()} == {
            "USER_PINS": state / "data" / "user_pins.csv",
            "AUTH_STATE": state / "data" / "auth_state.json",
            "PENDING_UPLOADS": state / "data" / "pending_uploads.json",
        }
        assert Path(values["UI_PATHS"]["VIDEOS"]) == base / "ui" / "media" / "videos"

    def test_device_profile_is_independent_of_software_location(self, tmp_path: Path) -> None:
        base = tmp_path / "release" / "main"
        device = tmp_path / "device-2"
        values = read_isolated_constants({
            "KNEESPA_BASE_DIR": str(base), "KNEESPA_DEVICE_DIR": str(device),
        })
        state = device / "raspberry-pi"
        assert Path(values["CONFIG_PATH"]) == state / "config" / "kneespa.cfg"
        assert Path(values["LOG_DIR"]) == state / "logs"
        assert Path(values["DATA_PATHS"]["USER_PINS"]) == state / "data" / "user_pins.csv"
        assert Path(values["UI_PATHS"]["VIDEOS"]) == base / "ui" / "media" / "videos"
    ...
    def test_numeric_pulse_cadence_enabled_by_default(self):
        ...
        result = subprocess.run(
            [
                ...
                "-c",
                (
                    "import sys; sys.path.insert(0, 'runtime/raspberry-pi/main'); "
