...
# C/C++ compiler and the vendored Unity framework in unity/.
set -euo pipefail
...
"$CC" -std=c99 -c unity/unity.c -I unity -o "$BUILD_DIR/unity.o"

status=0
for suite in test_*/; do
    name=$(basename "$suite")
    src=("$suite"*.cpp)
    bin="$BUILD_DIR/$name"
    echo "=== Building $name ==="
    if ! "$CXX" -std=c++11 -DUNIT_TEST -Wall -I unity -o "$bin" "${src[@]}" "$BUILD_DIR/unity.o"; then
        status=1
        continue
    fi
    echo "=== Running $name ==="
