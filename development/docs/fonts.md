...
`ui/theme/qss.py:load_fonts()` registers **every `.ttf` / `.otf`** in
`runtime/raspberry-pi/main/ui/theme/fonts/` with Qt at startup (`QFontDatabase.addApplicationFont`).

## Bundled (IBM Plex, SIL OFL 1.1 — see `runtime/raspberry-pi/main/ui/theme/fonts/LICENSE.txt`)
...
To update, drop replacement TTFs in the runtime font directory — they load automatically, no code change.
