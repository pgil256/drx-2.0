...
find_video() {
    ...
    for candidate in "$APP_DIR"/runtime/raspberry-pi/main/ui/media/videos/*.mp4; do
        if [[ -f "$candidate" ]]; then
            printf '%s\n' "$candidate"
            return
        fi
...
play_video_audio() {
    ...
    if [[ -z "$VIDEO_PATH" ]]; then
        echo "No KneeSpa MP4 file was found under runtime/raspberry-pi/main/ui/media/videos."
        return 2
    fi
